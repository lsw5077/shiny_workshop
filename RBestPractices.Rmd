...
Best practices for R graphics takes up [entire books](https://r-graphics.org/). We're just going to touch briefly on graphics here to provide some guidance for accessibility. Specifically, we'll deal with making graphics color-blind safe and making our graphics and code in general screen-reader-friendly. 

First, let's address colors. Color can be an important tool for data visualization, but we need to make sure our great colors are actually interpretable to everyone. Colors are composed of hues (the wavelength of light) and value (the ammount of black in the color). By choosing colors that contrast values as well as hues, we can make sure that color-blind audience members can interpret our visuals.  
