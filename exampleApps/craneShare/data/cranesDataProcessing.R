...
# cranes from ebird, raw data not included b/c it's too large
...
# Slim down cranes dataset, to just 2020 to make the app faster

ecranes <- read.csv("ecranes.csv")

ecranes <- ecranes %>%
           filter(year(as.Date(OBSERVATION.DATE)) == "2020")

write.csv(ecranes, "ecranes.csv")
